...
void CSoundMgr::PlayLoop(TCHAR* pSoundKey, float fVolume, CHANNELID eID)
{
	map<TCHAR*, FMOD_SOUND*>::iterator iter;
...
void CSoundMgr::PlayDamageSound(float fVolume)
{
	int r = rand() % 10;
	const wchar_t characters[] = L"abcdefghij";
	wchar_t character = characters[r];
	wchar_t result[50]; 
	swprintf(result, 50, L"dmg_normal_%c.wav", character);
	StopSound(CHANNELID::SOUND_EFFECT);
	PlaySound(result,CHANNELID::SOUND_EFFECT,fVolume);
}
